...
describe ('POST /products as admin', function () {
  ...
  request (app)
    ...
    .end (function (err, res) {
      ...
      expect (Array.isArray (res.body.errors)).toEqual(true)
      expect (res.body.errors).toEqual (
        expect.arrayContaining (['Product name required', 'Image Url must be in URL format', 'Price must be equal or above 0', 'Price must be equal or above 0'])
      )
      done ()
